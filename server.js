...
if (p === '/api/ops' && req.method === 'GET') {
  ...
  if (p === '/api/clients' && req.method === 'POST') {
    // ЗАПРЕЩЕНО: экспедитор не создаёт клиентов сам — только выбирает из уже существующих.
    // Раньше это было разрешено кому угодно кроме поваров, и агент плодил дубли на каждой точке
    // (опечатки, разное написание одного и того же магазина). Клиентов заводит менеджер/директор,
    // либо они приходят синхронизацией с 1С (/api/1c/clients-diff), где есть code1c и дублей не бывает.
    if (isCook(role) || isAgent(role)) return json(res, 403, { error: 'Нет прав добавлять клиентов' });
    const name = String(data.name || '').trim();
